...
写完之后重启一下服务，可以先使用get方法进行调试，访问```域名/settings/getinfo```看能否返回字典信息，能返回再进行下面的操作
...
```python
from django.http import JsonResponse
from django.contrib.auth import authenticate, login	# 用来验证用户有没有登录成功
